...
**当前状态：已完成运行时、网页工作台和离线回归，并通过 DeepSeek 小样例修复、多轮追问及只读流式验收。** 离线模式的下一步调用来自预设脚本；live 模式才由模型根据工具结果选择后续动作。此仓库不宣称生产部署或模型任务成功率。
...
打开终端输出的本地地址（包含一次生成的访问凭据）。创建任务“修复加法错误并测试”，发送任务后自动运行，查看 diff 后点击“批准并继续”；测试执行需要单独批准，决定提交后同样自动继续。此例只运行项目自建的示例代码。`trusted-local` 会在宿主机执行仓库测试，**不是沙箱**。默认后端为 `disabled`。
...
本地 Windows：51 项测试，50 通过、1 项因无法创建符号链接跳过；8 个离线集成场景通过，包含真实临时文件修改及测试子进程。网页通过登录、SSE 实时文字、审批自动继续、刷新恢复、会话搜索、退出及移动端无横向溢出检查。

这些结果验证机制，不衡量模型的独立规划或代码生成能力。`evaluation/offline-results.json` 记录数据 SHA-256、模式和逐项结果；另有少量 DeepSeek 实机冒烟测试：加法修复通过两个测试并能回答后续追问；只读流式任务完成 2 轮调用、产生 9 条文本增量。仅证明接入及基础闭环，不代表复杂任务成功率。详情见 [验收记录](docs/acceptance.md)。

## 边界

仅面向本机单操作人的仓库工作台，默认绑定 `127.0.0.1`，浏览器使用 HttpOnly / SameSite=Strict 会话 Cookie，API 保留 Bearer 兼容；跨站来源被拒绝。工作区锁为单进程内锁，不支持多 worker 互斥或多租户服务。
...
- 独立登录页；启动链接自动换取浏览器会话，随后清除地址栏中的凭据。手动登录输入的是工作台访问密码，不是模型 API Key。刷新无需重复登录；退出或服务重启后需要重新登录。
- 左侧为可搜索的会话列表，中央为对话及固定输入区，右侧展示待审批事项与实时工具轨迹。发送任务/追问即执行。
- 服务端通过 SSE 推送持久事件，模型通过 Chat Completions streaming 输出正文片段；不展示私有思考过程，工具参数完整收齐后才执行。
- 批准或拒绝当前待决操作后自动续跑。调度器串行处理同一工作区的执行，重复运行请求不会重复启动任务；重复审批返回冲突。
- 断线使用事件 ID 续接，前端去重；已输出部分内容后网络失败不会自动重放模型请求，避免半截工具调用被执行。

操作与验证见 [工作台升级说明](docs/workbench-v2.md)。
